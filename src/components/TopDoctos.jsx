...
const TopDoctors = () => {
  ...
  useEffect(() => {
    // This would normally come from your AppContext
    const mockDoctors = Array(10).fill().map((_, index) => ({
      _id: `doc-${index}`,
      name: `Dr. ${['Sarah Smith', 'John Davis', 'Emily Chen', 'Michael Wong', 'Jessica Taylor', 'Robert Johnson', 'Maria Garcia', 'David Kim', 'Lisa Patel', 'James Wilson'][index]}`,
      speciality: ['Cardiologist', 'Neurologist', 'Dermatologist', 'Pediatrician', 'Orthopedic Surgeon', 'Psychiatrist', 'Ophthalmologist', 'Gynecologist', 'Urologist', 'Gastroenterologist'][index],
  ...
  return (
    ...
    >
      ...
      >
        Connect with certified specialists and explore our wide range of
        specialties
      </p>
          ...
          return (
            ...
            >
              <div className="relative overflow-hidden">
                <img className={`bg-gradient-to-br ${color.light} w-full h-48 object-cover`} src={item.image} alt={item.name} />
                ...
                <div className={`mt-2 flex items-center justify-between transition-all duration-300 ${
                  ...
                  <div className="flex items-center">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <svg 
                        key={star} 
                        className={`w-4 h-4 ${star <= Math.floor(item.rating) ? 'text-amber-400' : 'text-gray-300'}`} 
                        fill="currentColor" 
                        viewBox="0 0 20 20"
                      >
                  ...
                  <span className="text-xs text-gray-600">{item.experience} yrs exp</span>
                </div>
              </div>
